...
# -------------------------------
df_notes = pd.read_csv("./data/teacher_notes_500.csv")  # يحتوي على: teacher_note, label
...
#df_notes[['student_id', 'RiskLevel']].to_csv("teacher_risk_levels_arabert.csv", index=False)



# 1- loding File
df = pd.read_csv("./data/synthetic_students_500_AI_.csv", dtype={"AI_RecommendedAction": "object"})
df['RiskLevel'] = clf.predict(X_embeddings)
df.to_csv("synthetic_students_500.csv", index=False)
...
df['RiskLevel'] = df['RiskLevel'].factorize()[0] # it give num for each catagoary
